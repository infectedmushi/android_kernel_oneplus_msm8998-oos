...
MODULES_DIR=/mnt/Building/AnyKernel2-render/modules/vendor/lib/modules
...
## copy assets
#cp -v *.ko /mnt/Building/AnyKernel2-render/modules/vendor/lib/modules
cd $KBUILD_OUTPUT/arch/arm64/boot
cp -v Image.gz-dtb /mnt/Building/AnyKernel2-render/zImage
cd /mnt/Building/AnyKernel2-render
zip -r9 RZ-$VARIANT-$(date +%y%m%d)-upstream.zip *
mv -v RZ-$VARIANT-$(date +%y%m%d)-upstream.zip /mnt/Building/Out_Zips
echo "Done!!!"
