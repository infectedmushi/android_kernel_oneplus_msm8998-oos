...
MODULES_DIR=/mnt/Building/repack/modules

cd /mnt/Building/oos-pie-kernel-stable
export USE_CCACHE=1
export VARIANT="OP5T-OOS-P"
export HASH=`git rev-parse --short=8 HEAD`
export KERNEL_ZIP="Stock+-$VARIANT-$(date +%y%m%d)-$HASH"
...
## copy assets
#cp -v *.ko /mnt/Building/repack/modules
cd $KBUILD_OUTPUT/arch/arm64/boot
cp -v Image.gz-dtb /mnt/Building/repack/zImage                     
cd /mnt/Building/repack
zip -r9 Stock+-$VARIANT-$(date +%y%m%d)-$HASH.zip *
mv -v Stock+-$VARIANT-$(date +%y%m%d)-$HASH.zip /mnt/Building/Out_Zips
echo "Done!!!"
