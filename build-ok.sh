...
cd /mnt/Building/infected-kernel-dev
export HASH=`git rev-parse --short=8 HEAD`
export LOCALVERSION=~infected-blu-r1-oos-ob23-pie-$(date +%y%m%d)-$HASH
export ARCH=arm64
...
zip -r9 infected-blu-r1-oos-ob23-pie-op5t-$(date +%y%m%d)-$HASH.zip *
mv -v infected-blu-r1-oos-ob23-pie-op5t-*.zip /mnt/Building/Out_Zips
echo "Done!!!"
