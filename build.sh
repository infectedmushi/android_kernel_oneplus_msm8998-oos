...
export USE_CCACHE=1
export VARIANT="OP5T-OOS-P-OB23"
export HASH=`git rev-parse --short=8 HEAD`
export KERNEL_ZIP="StockPlus-$VARIANT-$(date +%y%m%d)-$HASH"
...
cp -v Image.gz-dtb /mnt/Building/AnyKernel2-stock/zImage                     
cd /mnt/Building/AnyKernel2-stock
zip -r9 StockPlus-$VARIANT-$(date +%y%m%d)-$HASH.zip *
mv -v StockPlus-$VARIANT-$(date +%y%m%d)-$HASH.zip /mnt/Building/Out_Zips
echo "Done!!!"
