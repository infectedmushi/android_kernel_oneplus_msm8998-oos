...
#MODULES_DIR=/mnt/Building/AnyKernel2/
KERNEL_DIR=/mnt/Building/infected-kernel
cd $KERNEL_DIR
export USE_CCACHE=1
export VARIANT="oos-r4-pie"
export HASH=`git rev-parse --short=8 HEAD`
export KERNEL_ZIP="infected-$VARIANT-$(date +%y%m%d)-$HASH"
...
zip -r9 infected-$VARIANT-$(date +%y%m%d)-$HASH.zip *
mv -v infected-$VARIANT-$(date +%y%m%d)-$HASH.zip /mnt/Building/Out_Zips
echo "Done!!!"
